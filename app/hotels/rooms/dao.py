...
class RoomDAO(BaseDAO):
    ...
    async def find_all(cls, hotel_id, date_from, date_to):
        ...
        async with async_session_maker() as session:
            subquery_booked_rooms = select(Booking.room_id).where(
                ((Booking.date_from >= date_from) & (Booking.date_from < date_to)) |
                ((Booking.date_from <= date_from) & (Booking.date_to > date_from))
            ).cte('booked_rooms')
            
            subquery_rooms_left = select(
                Room.id.label('room_id'), 
                (Room.quantity - func.count(subquery_booked_rooms.c.room_id)).label('left')
            ).join(
            ...
            rooms_in_hotel = select(
                Room.__table__.columns, 
                subquery_rooms_left.c.left,
                (Room.price * (date_to - date_from).days).label('total_coast')
            ).join(
                subquery_rooms_left,
                Room.id == subquery_rooms_left.c.room_id
            ).where(
                Room.hotel_id == hotel_id
            )

            print(rooms_in_hotel.compile(engine, compile_kwargs={"literal_binds": True}))
